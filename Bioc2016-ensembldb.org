...
+ _Example_: plot genes encoded on a chromosomal region using =Gviz=.
  ...
  #+NAME: usage-gviz
  #+BEGIN_SRC R :results output graphics :file gviz-ska.pdf :exports both
    library(Gviz)
...
+ _Example_: use =ggbio= and =ensembldb= to plot a chromosomal region.
  ...
  #+NAME: usage-ggbio
  #+BEGIN_SRC R :results output graphics :file ggbio-ska2.pdf :exports both
    ## Use ggbio:
    library(ggbio)
...
Get samples: M003 P7 Adipo and V2P8: M003 tmes L001 -> A.bam V2P8 tmes L001 -> B.bam


#+BEGIN_SRC R :eval never
  ## Check expression of SKA2 and of F2RL1.
  load("data/RData/example-GeneCounts.RData")
  load("data/RData/tmp-geneCounts-all-chrom.RData")

  gns <- c("SKA2", "F2RL1")
  gns <- genes(edb, filter=GenenameFilter(gns))
  ## They are on chr 17 and 5.

  assay(GeneCounts)[names(gns), ]

  tmp <- assay(GeneCountsReduced)[names(gns), ]

  ## Size factor:
  sf <- colSums(assay(geneCounts))/mean(colSums(assay(geneCounts)))
  tmp <- assay(geneCounts)[names(gns), ]
  tmp / sf
