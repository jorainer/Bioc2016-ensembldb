...
- For exons:
  + =ExonidFilter=
  + =ExonrankFilter=
- Based on chromosomal coordinates:
  + =SeqnameFilter=
  + =SeqstrandFilter=
  + =SeqstartFilter=
  + =SeqendFilter=
...
** Some internals on the filter framework                          :ARCHIVE:

+ How does the filter framework work?
+ _Example_: the =where= method of a =GenenameFilter=.
...
+ Implementing this method for other database objects enables reuse of the
