...
* `EnsDB` object/database:
    +   Designed for Ensembl annotations: **all** genes, new attributes *gene biotype* and *tx biotype.*
...
**Example**: get all genes or transcripts.
...
```{r message=FALSE}
## Alternatively, we can specify to return the results as a DataFrame
...
- Use filters to have more control of the query.
...
-   `EnsDb` provide gene model information for feature counting: `exonsBy`.
...
-   UCSC and Ensembl use different chromosome name formats.
...
-   __Example__: get SKA2 tx sequences using BSgenome.
...
-   We can build an `EnsDb` from an `AnnotationHub` object.
...
-   __Example__: implementation in the `mirtarbase` apcakge.
    - https://github.com/jotsetung/mirtarbase
    - Bases on the miRTarBase data: validated miRNA target genes (http://mirtarbase.mbc.nctu.edu.tw/).
...
## Get the ID of the publications.
pmid(ska2mirs)
```
